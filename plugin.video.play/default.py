...
kodi_path = TRANSLATEPATH('special://home/addons/script.module.beautifulsoup4/lib')
sys.path.append(kodi_path)
...
# Verificar si la carpeta skin.xonfluence existe en addon_data

if not os.path.exists(skin_xonfluence_folder):        
                      #con esto empieza el modulo comprobando si es una primera instalacion
    xbmc.executebuiltin('RunScript(special://home/addons/plugin.video.play/resources/lib/inicio.py)')
        
#xbmcgui.Dialog().ok("Resultado", "volvemos de inicio.py.")        
#time.sleep(0.35)
...
def addon_log(string, level=xbmc.LOGDEBUG):
    if debug == 'true':
        xbmc.log("[plugin.video.play-{0}]: {1}".format(addon_version, string), LOGINFO)
    else:
        xbmc.log("[plugin.video.play-{0}]: {1}".format(addon_version, string), level)
...
def getSources():
    #el modulo principal ejecuta esta funcion la primera
    
    #llamador = inspect.stack()[1][3]
    #xbmcgui.Dialog().textviewer("estamos en getSources con el XML", str("llamador"))
    try:
        if os.path.exists(favorites):
...
def addSource(url=None):
    ...
    if media_info:
        ...
        keyboard = xbmc.Keyboard(nameStr, 'Nombre mostrado, ¿Cambiar nombre?')
        keyboard.doModal()
...
def getSoup(url, data=None):
    ...
    from bs4 import BeautifulSoup
    #xbmcgui.Dialog().textviewer("estoy dentro de getSoup", str(url))    
    global viewmode, tsdownloader, hlsretry, ordenar_capitulos
    
        #xbmc.Player().stop()
        #xbmcgui.Dialog().ok("formado el xml.", url)
    tsdownloader = False
    hlsretry = False
    ...
    if url.startswith('http://') or url.startswith('https://'):
        ...
        #xbmcgui.Dialog().textviewer("Primera data", data)
        if enckey:
    ...
    if "cj4pim/sal" in url:
        contador_topuria = 1
        ruta_topuria = TRANSLATEPATH('special://home/addons/plugin.video.play/resources/lib/contador.txt')        
        with open(ruta_topuria, "w") as f:
            f.write(str(contador_topuria))
    if "21/12/pu" in url:
...
###una vez instalado a ver si funciona este xbmc.executebuiltin(f'UpdateAddon({addon_id})')                    
            ...
            def check_for_update(repoid):
                ...
                if latest_version is None or installed_version is None:
                    ...
                    if "resol" in repoid:
                        paso_update = 1
                        dialog.ok("Kodi se cerrará automáticamente", "Resolverdor de enlaces necesita esta versión: {}".format(latest_version))
              #         URL de descarga de la actualización
                        update_url = "https://raw.githubusercontent.com/Gujal00/smrzips/master/zips/script.module.resolveurl/script.module.resolveurl-{}.zip".format(latest_version)
              ...
              #         Eliminar el archivo de actualización descargado y cerrar kodi
                    ...
                    if "Macke" in repoid:
                        ...
                        dialog.ok("Kodi se cerrará automáticamente", "Actualizar el repo a esta versión más reciente: {}".format(latest_version))
              #         URL de descarga de la actualización
        ...
        comprobar_repo("https://raw.githubusercontent.com/Gujal00/smrzips/master/addons.xml", "script.module.resolveurl")
        ...
        # Sección problemática: Creación de objeto BeautifulSoup
    soup = BeautifulSoup(data, 'html.parser')
    #xbmcgui.Dialog().textviewer("Pagina en bruto", str(data))
    # Buscar la etiqueta <SetViewMode>
    set_view_mode_tag = soup.find('setviewmode')
    modo_ordenar = soup.find('pororden')
    if modo_ordenar:
        ordenar_capitulos_text = modo_ordenar.text
        ordenar_capitulos = int(ordenar_capitulos_text)
    # Verificar si la etiqueta <SetViewMode> fue encontrada
    if set_view_mode_tag:
        # Obtener el contenido de la etiqueta <SetViewMode>
        viewmode_text = set_view_mode_tag.text
        viewmode = int(viewmode_text)
        #xbmcgui.Dialog().textviewer("Pagina en bruto", str(viewmode))
        
    items = soup.find('items')

    
    #items = html_contenido.find('items')
    #xbmcgui.Dialog().textviewer("Pagina en bruto", html_contenido)    
    if items:
        item_tags = items.find_all('item')
        xml_data = '\n'.join(str(tag) for tag in item_tags)
        xml_data = re.sub(r"<link/>(.*)", r"<link>\g<1></link>", xml_data)
        xml_data = "<items>" '\n' + xml_data + '\n' "</items>"
        xml = ElementTree.fromstring(xml_data)
        #xbmcgui.Dialog().textviewer("formado el xml", str(xml_data))
        return xml
    #      
    #
    
    
    #xbmcgui.Dialog().textviewer("Pagina en bruto", str(soup))
    return soup
...
def getData(url, fanart, data=None):
    #a este le llama get sources
    #llamador = inspect.stack()[1][3]
    #xbmcgui.Dialog().textviewer("estamos en getData con la url", str(url))
    #xbmc.log("estoy dentro de getData ", xbmc.LOGINFO)
    global paginacion_kodi, siguiente_llamada
    #xbmcgui.Dialog().textviewer("estoy dentro de getData", str(url))
    #xbmc.log("FUNCTION 'GETITEMS' FUE LLAMADA POR:" + url, xbmc.LOGINFO)
    if "com/s/ovgt5758ps0f91o/" in url:
        siguiente_llamada = 'fanart=https%3a%2f%2fdl.dropboxusercontent.com%2fscl%2ffi%2fqtemtvgmvtu4q426w509g%2f235833.jpg%3frlkey%3do3p2cqmbq4tcxguog85xtwr9t%26dl%3d0&mode=1&name=%5bCOLOR%20orangered%5d%5bB%5dPelicu%5b%2fCOLOR%5d%5bCOLOR%20gold%5dlas%20Esp%5b%2fCOLOR%5d%5bCOLOR%20orangered%5da%c3%b1olas%5b%2fB%5d%5b%2fCOLOR%5d&url=https%3a%2f%2fdl.dropbox.com%2fs%2fovgt5758ps0f91o%2fPeliculas_Espa%25C3%25B1olas.html'
        paginacion_kodi = 'A_paginar'
    soup = getSoup(url, data)
    #xbmcgui.Dialog().textviewer("estamos en getData con la url", str(soup))
    dialog = xbmcgui.Dialog()           
    ...
    if isinstance(soup, ElementTree.Element):
        ...
        if channels is None or len(channels) == 0:
            #xbmcgui.Dialog().textviewer("voy hacer una lista desde getDATA para enviarsela a getITEMS", str(soup))
...
def parse_m3u(data):
    #xbmcgui.Dialog().textviewer("estoy dentro de parse_m3u", str(data))
    content = data.rstrip()
...
def getChannelItems(name,url,fanart):
        ...
        for channel in channel_list('subchannel'):
            ...
            try:
                addDir(name.encode('utf-8', 'ignore'),3,thumbnail,fanArt,desc,genre,credits,date)
            except:
...
def getSubChannelItems(name, url, fanart):
    #xbmcgui.Dialog().textviewer("estoy dentro de getSubChannelItems", str(name))
    soup = getSoup(url)
...
def getItems(items, fanart, dontLink=False):
    #xbmc.log("estoy dentro de getItems " + "  " + str(items), xbmc.LOGINFO)
    #xbmcgui.Dialog().textviewer("estoy dentro de getitems", str(items))
    #### a este le llama getData
    #llamador = inspect.stack()[1][3]
    #xbmcgui.Dialog().textviewer("estamos en addDir con el XML", str(llamador))
    total = len(items)
######################################################################################################################
    ###caller_info = inspect.stack()[1]
    ###xbmc.log(f"FUNCTION 'GETITEMS' FUE LLAMADA POR:", xbmc.LOGINFO)
    #xbmc.log(f"File: {caller_info.filename}", xbmc.LOGINFO)
    ###xbmc.log(f"FUNCTION: {caller_info.function}", xbmc.LOGINFO)
    ###xbmc.log(f"LINE: {caller_info.lineno}", xbmc.LOGINFO)    
    #xbmcgui.Dialog().ok("estoy dentro de getItems", str(items))
##########################################################################################################
    #xbmcgui.Dialog().ok("estoy dentro de addLink", str(name))
    #xbmcgui.Dialog().ok("estoy dentro de getItems", str(items))
    ...
    for item in items:
        ...
        if isinstance(item.find('title'), ElementTree.Element):
            ...
            #xbmcgui.Dialog().ok("estoy dentro de getItems", name)
            if name == '':
                name = 'unknown?'
            name = processPyFunction(name)
        else:
        ...
        try:
            if len(url) > 1:
                ...
                if isXMLSource:
                    ...
                    #xbmcgui.Dialog().textviewer("estoy dentro de getItems", name)
...
def parse_regex(reg_items):
    #xbmcgui.Dialog().textviewer("estoy dentro de parse_regex", str(reg_items))
...
def getRegexParsed(regexs, url, cookieJar=None, forCookieJarOnly=False, recursiveCall=False, cachedPages={}, rawPost=False, cookie_jar_file=None):
    #xbmc.log(str(regexs) + "   " + str(url), xbmc.LOGINFO) 
    #xbmcgui.Dialog().ok("estoy drento de getRegexParsed", str(regexs) + "   " + str(url))
    pelis_por_ver = 0
    if url == "$doregex[pelis_por_ver]":
        pelis_por_ver = 1
    ...
    for k in doRegexs:
        if k in regexs:
            ...
            if m.get('page') and '$doregex' in m.get('page'):
            ...
            link = ''
            if m.get('page') and m.get('page') in cachedPages and 'ignorecache' not in m and not forCookieJarOnly:
                link = cachedPages[m['page']]
            else:
                
                if m.get('page') and m.get('page') != '' and m.get('page').startswith('http'):
                    ...
                    try:
                        ...
                        if encoding is not None:
                            ...
                            link = link.decode('latin-1', errors='ignore')
                ...
                elif m.get('page') and not m.get('page').startswith('http'):
                    
                    if m['page'].startswith('$pyFunction:'):
                        val = doEval(m['page'].split('$pyFunction:')[1], '', cookieJar, m)
                        if forCookieJarOnly:
                            return cookieJar  # no hacer nada
                        link = val
                        link = javascriptUnEscape(link)
                    else:
                        link = m['page']
            ...
            if m['expres'] != '':
                if '$LiveStreamCaptcha' in m['expres']:
                    ...
                    #link = link.decode('utf-8')  # Convertir bytes a cadena de texto

                    if 'listrepeat' in m:
                        listrepeat = m['listrepeat']
                        ret = re.findall(m['expres'], link.decode('utf-8'))  # Utilizar el patrón como cadena de texto
                        return listrepeat, ret, m, regexs, cookieJar

                    val = ''
                    if link != '':
                        reg = re.compile(m['expres']).search(link.decode('utf-8'))  # Utilizar el patrón como cadena de texto
                        if reg:
                            val = reg.group(1).strip()

                    elif m.get('page') == '' or m.get('page') is None:
                        val = m['expres']
                    ...
                    try:
                        url = url.replace("$doregex[" + k + "]", val)
                        #### val seria la url resuelta con su cabeza si la tuviera
                        #xbmc.log(str(regexs) + "   " + str(url), xbmc.LOGINFO) 
                        #xbmcgui.Dialog().ok("estoy drento de getRegexParsed", str(url))
                    except:
                        url = url.replace("$doregex[" + k + "]", val)

            else:
                #xbmcgui.Dialog().ok("estoy en else antes de replace", str(url))
                url = url.replace("$doregex[" + k + "]", '')   
    #xbmc.log(str(regexs) + "   " + str(url), xbmc.LOGINFO)
    #xbmcgui.Dialog().ok("estoy en funcion a ver que url", str(url))
    if '$epoctime$' in url:
    ...
    if url == "":
        if pelis_por_ver == 1:
            ...
            if es_android:
                #xbmcgui.Dialog().ok("es_android", str(es_android))
                time.sleep(1)
            else:                 
                url = "C:\\Users\\Asus_Tv\\AppData\\Roaming\\Kodi\\addons\\plugin.video.play\\resources\\lib\\por_ver.html"
            #xbmcgui.Dialog().ok("estoy en funcion valor 1", str(url))
...
def getUrl(url, cookieJar=None, post=None, timeout=20, headers=None, noredir=False):
    cookie_handler = urllib_request.HTTPCookieProcessor(cookieJar)
    #xbmc.log("estoy dentro de getUrl " + str(url), xbmc.LOGINFO)
    if post is not None:
...
def urlsolver(url):
    ...
    if resolveurl.HostedMediaFile(url).valid_url():
        ...
        xbmcgui.Dialog().notification(addon_name, 'ResolveUrl, no es compatible con este dominio.', icon, 5000, False)
        sys.exit()
        resolved = url
    return resolved
...
def play_playlist(name, mu_playlist, queueVideo=None):
    #xbmc.log("estoy dentro de play_playlist ", xbmc.LOGINFO)
...
def addDir(name, url, mode, iconimage, fanart, description, genre, date, credits, showcontext=False, regexs=None, reg_url=None, allinfo={}):
    #################################################################################
    #da valor a page desde kodi para saber en que pagina esta en peliculas españolas
    ...
    #xbmc.log("estoy dentro de addDir " + str(name) + "  " +str(url) + "  " +str(mode), xbmc.LOGINFO)
    #xbmcgui.Dialog().textviewer("estamos en addDir con el XML", str(name) + "  " +str(url) + "  " +str(mode))
    ...
    """
    #este se encarga solo delos externalink y de los jsonrpc, quien llama es getitem
    #llamador = inspect.stack()[1][3]
    #
    ...
    if showcontext:
        ...
        if showcontext == 'source':
            ...
            contextMenu.append(('Remove from Play Favorites', 'RunPlugin(%s?mode=6&name=%s)'
        ...
        if name not in FAV:
            contextMenu.append(('Add to Play Favorites', 'RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
    ...
    #xbmc.log("URL es...: " + u, xbmc.LOGINFO)
    
    ok = xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=u, listitem=liz, isFolder=True)
        
    
    #en esta ocasion necesitamos un tiempo de espera para cambiar el modo de visualizacion
    #url.startswith("plugin://plugin.video.palantir3/?Gb1NWasVGccxVYp")
    #
    
    #xbmc.log("URL es...: " + u, xbmc.LOGINFO) 
    #xbmcgui.Dialog().textviewer("URL es...: ", u)
    if ok and "3FbhZGXcNjc" in u:        
        tiempo_espera = 0.5
        threading.Timer(tiempo_espera, cambiar_modo_de_vista).start()                   
    return ok

def cambiar_modo_de_vista():
    xbmc.executebuiltin("Container.SetViewMode(732)")    
    return
...
def sendJSON(command):
    ...
    #xbmcgui.Dialog().ok("estamos en sendJSON", str(command))
    data = ''
    try:
    ...
    #xbmc.log("estamos en sendJSON " + str(data) , xbmc.LOGINFO)
    #xbmcgui.Dialog().ok("estamos en sendJSON", str(data))
    return uni(data)


def pluginquerybyJSON(url, give_me_result=None, playlist=False):
    #xbmcgui.Dialog().textviewer("pluginquerybyJSON", str(mode) + "\n" + "\n".join([url[i:i+83] for i in range(0, len(url), 83)]))
    #xbmcgui.Dialog().textviewer("estamos en pluginquerybyJSON", str(url))
...
contador_Transformers = 0
def cambiar_vista_Transformers():
...
total_elementos = 0
dict_items = {}
variables_bucle = False
cuentas_1 = False
import math

def addLink(url, name, iconimage, fanart, description, genre, date, showcontext, playlist, regexs, total, setCookie="", allinfo={}):
    #ESTA FUNCION ES PARA EL EL LINK DIRECTO A LA PELICULA
    #a esta funcion le llama getItem
    #llamador = inspect.stack()[1][3]
    #xbmcgui.Dialog().textviewer("estamos en addLink con el XML", str(total))
    #xbmcgui.Dialog().ok("estoy dentro de addLink", str(name))
    
#########################################################################PARA SABER QUIEN LLAMA A ESTA FUNCION    
    ###caller_info = inspect.stack()[1]
    ###xbmc.log(f"Function 'addLink' was called from:", xbmc.LOGINFO)
    #xbmc.log(f"File: {caller_info.filename}", xbmc.LOGINFO)
    ###xbmc.log(f"Line: {caller_info.lineno}", xbmc.LOGINFO)
    ###xbmc.log(f"Function: {caller_info.function}", xbmc.LOGINFO)
    #xbmc.log(f"Value of 'name': {name}", xbmc.LOGINFO)
    #xbmcgui.Dialog().ok("estoy dentro de addLink", str(name))
###############################################################################################################

    #xbmc.log("estoy dentro de addLink " + name + "   " +str(url) , xbmc.LOGINFO)
    ...
    if name and name != '':        
        #xbmcgui.Dialog().ok("estoy dentro de addLink", name)
        u += "&nombre=" + urllib_parse.quote_plus(name)
    ...
    #xbmc.log(str(liz), xbmc.LOGINFO)
    #xbmcgui.Dialog().ok(str(u), str(liz))
    #if not pagina_procesada:
        
        
    #else:
    global contador_paginacion, dict_items, cuentas_1, page, variables_bucle, enlaces_diretos, start_index, end_index, contar_ultima, ITEMS_FIRST_PAGE, ITEMS_OTHER_PAGES, sin 
        
    if paginacion_kodi != 'A_paginar':
        ok = xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=u, listitem=liz, totalItems=total, isFolder=isFolder)
    else:
        if not variables_bucle:
            enlaces_diretos = total - 3 #Si agragas un exterallink a peliculas españolas seria entonces un -4 en la variable enlaces_diretos, son -3 por 2 externallink y el boton de next                           
            params = dict(parse_qsl(sys.argv[2][1:])) if len(sys.argv) > 2 else {} 
            page = int(params.get('page', 1))
            
            ITEMS_FIRST_PAGE = 104
            ITEMS_OTHER_PAGES = 106
            if page == 1:
                # Si es la primera página, mostramos 104 elementos
                start_index = 0
                end_index = ITEMS_FIRST_PAGE
            else:
                # Si es cualquier otra página, mostramos 106 elementos por página
                start_index = ITEMS_FIRST_PAGE + (page - 2) * ITEMS_OTHER_PAGES
                end_index = start_index + ITEMS_OTHER_PAGES
                
            resultado = enlaces_diretos / 105
            parte_entera = int(resultado)
            parte_decimal = round((resultado - parte_entera), 2)
            # Calculamos el valor límite dinámicamente
            decimal_limit = 0.02 + (page - 3) * 0.01 if page >= 3 else 0.02
            decimal_page = round((page/100-0.01), 2)
            #xbmcgui.Dialog().ok("page", f"decimal_limit={decimal_limit} y el decimal_page={decimal_page} y la pagina es={page}")
             
   ###########AQUI ES DONDE TRANSICIONA DE UNA PAGINA A OTRA#################
            if decimal_limit == decimal_page:
                if parte_decimal > decimal_limit:                    
                    #xbmcgui.Dialog().ok("page", "primer if")
                    contar_ultima = math.ceil(resultado)
                else:
                    corte = end_index - end_index
                    #xbmcgui.Dialog().ok("page", f"si, {end_index}, es menor a el total, {total} entonces me quedo y no sigo")
                    contar_ultima = math.floor(resultado)
                    if parte_decimal == decimal_limit and end_index > total:
                        #xbmcgui.Dialog().ok("page", f"como {end_index}, es mayor a {total}, pues pasamos de la pagina {page-1} a la pagina {page}")
                        contar_ultima = math.ceil(resultado)                    
            else:
                if parte_decimal >= decimal_limit:
                    #xbmcgui.Dialog().ok("page", "el segundo if")
                    contar_ultima = math.ceil(resultado)
                else:
                    #xbmcgui.Dialog().ok("page", "el segundo else")
                    contar_ultima = math.floor(resultado)
   ##############################################################################
            #sin = round((enlaces_diretos / 105), 2)           
            if end_index >= enlaces_diretos-1:
                end_index = enlaces_diretos+1
            variables_bucle = True
        
        dict_items[contador_paginacion] = {'url': u, 'listitem': liz}       
        contador_paginacion += 1      
            
  ######## ASI ENTENDI EL CAMBIO DE PAGINA        
        #if contador_paginacion >= 160:#en tu madre o la mia
            #xbmcgui.Dialog().ok("toy dentro", f'page={page}   start_index={start_index}   end_index={end_index}   contador_paginacion={contador_paginacion}   enlaces_diretos={enlaces_diretos}   total={total}            variable sin={sin}  contar_ultima={contar_ultima}')
  #############################################################
  ##este cacho lo pase al else del if que construye pagina con le boton next
        ###if page == contar_ultima:            
            ###if contador_paginacion == enlaces_diretos:
                               
                ###for i in range(start_index, end_index+1):#while i <= end_index:
                    ###item = dict_items[i]                   
                    ###xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=item['url'], listitem=item['listitem'], totalItems=len(dict_items), isFolder=False)
#########################################################################                    
                    
                
        ini_pag = start_index in dict_items
        fin_pag = end_index in dict_items
        #xbmcgui.Dialog().ok("numero_de_elementos", str(ini_pag) + '  '  +str(start_index) + '  '  + str(fin_pag) + '  '  +str(end_index) + '  '  + str(contador_paginacion - 1) + " total=" + str(total))
        if page != contar_ultima and ini_pag and fin_pag and start_index <= contador_paginacion - 1 and end_index >= contador_paginacion - 1:#numero_de_elementos == total_pagina:
            # 
            ###i = start_index
            #xbmcgui.Dialog().ok("page", f"con next y trabaja desde {start_index} hasta {end_index} y el diccionario tiene {contador_paginacion-1}")
            for i in range(start_index, min(end_index, contador_paginacion)):###while i < end_index and i < numero_de_elementos:
                                       
                item = dict_items[i]  # Accedemos al ítem correspondiente en el diccionario # ListItem
               
                xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=item['url'], listitem=item['listitem'], totalItems=len(dict_items), isFolder=False)                
                #i += 1
                
            if end_index < total:                
                siguiente_url = f"{sys.argv[0]}?{siguiente_llamada}&page={page + 1}"
                #xbmcgui.Dialog().ok("page", str(siguiente_url))
                siguiente_item = xbmcgui.ListItem(f'Página Siguiente')
                thumbnail_url = 'https://dl.dropbox.com/scl/fi/keg1q1pvojaluz3be9gte/siguiente.png?rlkey=agga4x3pv8rf4lfaxye33flxm&st=ssr1plu2&dl=0'  # URL de la imagen del Next
                siguiente_item.setArt({'thumb': thumbnail_url})
                xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=siguiente_url, listitem=siguiente_item, isFolder=True)
        else:
            if contador_paginacion == enlaces_diretos and page == contar_ultima:
                #xbmcgui.Dialog().ok("page", "sin next")
                for i in range(start_index, end_index+1):#while i <= end_index:
                    item = dict_items[i]                   
                    xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=item['url'], listitem=item['listitem'], totalItems=len(dict_items), isFolder=False)
                
    #xbmcgui.Dialog().ok("robocop", str(name))
    if any(movie in str(name) for movie in ["Transformers", "RoboCop", "Saw", "Rocky", "Matrix", "Rambo", "Acorralado", "Torrente", "Venom", "Transporter", "Jurassic", "Karate", "Los cazafantasmas", "Cazafantasmas", "Regreso al futuro", "Mad Max", "Misin", "Piratas", "Terminator", "Equalizer", "equalizer"]):
        #xbmcgui.Dialog().ok("robocop", str(name))
        global contador_Transformers
        contador_Transformers += 1
        if contador_Transformers == total:
            tiempo_espera = 0.3
            threading.Timer(tiempo_espera, cambiar_vista_Transformers).start()
            contador_Transformers = 0
    ...
    return ok 
        ...
        #xbmcgui.Dialog().textviewer("URL es...palantir3: ", url)
        #xbmc.log("URL es...: " + url, xbmc.LOGINFO)
    #xbmcgui.Dialog().ok("estamos en addLink", url) 
    ###if "[elpatron]" in url:
        ###global condicion_elpatron
        ###condicion_elpatron += 1
        ###if condicion_elpatron == total:         
            #xbmcgui.Dialog().ok("estoy dentro de addLink", str(condicion_elpatron))
            ###xbmc.executebuiltin("Container.SetViewMode(515)")
            ###xbmc.executebuiltin("Container.SetSortMethod(1)")
            ###condicion_elpatron = 0
        #xbmcgui.Dialog().ok("estoy dentro de addLink", str(con_elpatron) + "   " +str(url))
...
def playsetresolved(url, name, iconimage, setresolved=True, reg=None):
    #xbmc.log(url + "  " + str(name) + "  " + iconimage + "  " + str(setresolved) + "  " + reg, xbmc.LOGINFO)
    #xbmcgui.Dialog().ok("estoy dentro de playsetresolved", url + "  " + str(name) + "  " + iconimage + "  " + str(setresolved) + "  " + reg)
    if url is None:
        xbmcgui.Dialog().textviewer("estoy en url es none", url)
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
        return
    ...
    if setresolved:
        ...
        if reg and 'notplayable' in reg:
            setres = False
        #xbmcgui.Dialog().ok("estoy dentro de setresolved", str(name))
        liz = xbmcgui.ListItem(name)
        liz.setArt({'thumb': iconimage,
                    'icon': iconimage})
        ...
        if '&mode=19' in url:
            ...
            if '.m3u8' in url:
                ...
                liz.setContentLookup(False)
        #xbmc.log(str(url), xbmc.LOGINFO)        
        #xbmcgui.Dialog().ok("antes de tratar", str(url))
        liz.setPath(url)
        if not setres:
            ...
            # Ejecutar setResolvedUrl con la URL válida
            if url != 'DETENER':
                xbmcplugin.setResolvedUrl(int(sys.argv[1]), True, liz)
                xbmc.log("Reproducción iniciada correctamente.", xbmc.LOGINFO)
            else:
                xbmcplugin.setResolvedUrl(int(sys.argv[1]), False, xbmcgui.ListItem())
                threading.Timer(0, cerrar_dialogos).start()
                #sys.exit()
        

    else:
        #xbmcgui.Dialog().textviewer("estoy en ultimo else", url)
        xbmc.executebuiltin('RunPlugin(' + url + ')')
    #xbmcgui.Dialog().textviewer("estoy en la ultima linea", url)
...
if mode is None:
    ...
    data = None
    if regexs and len(regexs) > 0:
        #xbmc.log(str(regexs), xbmc.LOGINFO)
        #xbmcgui.Dialog().ok("estoy en mode 1", str(url))
        data, setresolved = getRegexParsed(regexs, url)
        #xbmc.log(str(setresolved), xbmc.LOGINFO)
        #xbmcgui.Dialog().ok("estoy en mode 1", str(data))
        if data.startswith('http') or data.startswith('smb') or data.startswith('nfs') or data.startswith('/') or data.startswith('C'):
            url = data
            data = None
...
#xbmcplugin.setResolvedUrl(int(sys.argv[1]), True, listitem=xbmcgui.ListItem(path=url))#con la url resuelta se la mandamos a kodi para que el reproductor la reproduzca
    ...
    #xbmcgui.Dialog().ok("estoy dentro de mode=17", str(url))
    ...
    #xbmcgui.Dialog().ok("estoy en mode=18", str(url))
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
    ...
    #xbmcgui.Dialog().ok("estoy en mode=19", str(url))
    addon_log("Genesiscommonresolvers")
    playsetresolved(urlsolver(url), name, iconimage, True)

elif mode == 20:
    #xbmcgui.Dialog().ok("estoy en mode =20", str(url))
    addon_log("setResolvedUrl")
...
if viewmode is not None:    
    #xbmcgui.Dialog().ok("estamos de viewmode", str(viewmode))
    es_android = xbmc.getCondVisibility("system.platform.android")
    if es_android:
        #xbmcgui.Dialog().ok("es_android", str(es_android))
        time.sleep(1)
        ...
        time.sleep(0.1)
    #xbmcgui.Dialog().ok("CONTADOR", str(contador_elpatron))
    xbmc.executebuiltin(f"Container.SetViewMode({viewmode})")
if ordenar_capitulos is not None:
    xbmc.executebuiltin(f"Container.SetSortMethod({ordenar_capitulos})")
