...
kodi_path = xbmc.translatePath('special://home/addons/script.module.beautifulsoup4/lib')
...
def getSoup(url, data=None):
    ...
    #xml_data = ""
    #if not url.startswith('http://') and not url.startswith('https://') and '<item>' in url:    
        #xbmcgui.Dialog().textviewer("dentro de que no es una url valida Y SI VE items ", str(url))
        
        #url_str = str(url)
        #url_element = ElementTree.fromstring(url_str)
        #xml_data = ElementTree.tostring(url_element).decode()
        
        
        #xml_start = xml_data.find('<items>')
        #xml_end = xml_data.find('</items>', xml_start)
        #if xml_start != -1 and xml_end != -1:
            
            #xbmcgui.Dialog().textviewer("estamos extrayendo datos ", str(url))            
            #xml_data2 = xml_data[xml_start:xml_end + len('</items>')]
            #xml_data2 = re.sub(r"<link />(.*)", r"<link>\g<1></link>", xml_data2)
            #xml_data2 = re.sub(r"<cookiejar />", "<cookieJar></cookieJar>", xml_data2)
            #xml_rectificada = '<items>'
            #matches = re.findall(r'<item>(.*?)</item>', xml_data2, re.DOTALL)
            
            #for match in matches:
                
                #xml_rectificada += '<item>' + match + '</item>'

            #xml_rectificada += '</items>'
            
            #try:
                #xml = ElementTree.fromstring(xml_rectificada) 
                #xbmcgui.Dialog().textviewer("Esto se envia a getITEMS pero en lista :)", str(xml_rectificada))
                #getItems(xml.findall('item'), fanart)
                #getData(url, fanart, xml)
                
                #########################
                #components = [item for item in xml]
                #xbmcgui.Dialog().textviewer("lo que sale realmente GetSoup element", str(components)) 
                #getItems(components, fanart)
    ...
    #xbmcgui.Dialog().textviewer("Pagina en bruto", data)    
    if items:
    ...
    #xbmcgui.Dialog().textviewer("SE COMUNICA BS4 A ELEMENT ", str(xml_data))      
    ...
    if xml_start != -1 and xml_end != -1:
        ...
        xml_rectificada += '</items>'
        #xbmcgui.Dialog().textviewer("Resultado element", xml_rectificada)
        try:
            xml = ElementTree.fromstring(xml_rectificada)
            #xbmcgui.Dialog().textviewer("lo que sale realmente (GetSoup-element)para getDATA", str(xml))
            return xml
        except ElementTree.ParseError:
            pass
        except Exception as err:
...
def getData(url, fanart, data=None):
    ...
    #dialog.ok('Valor devuelto por getSoup', str(soup))
    
    #if 'cookiejar' in str(soup):
        #str_soup = ""
        #xbmcgui.Dialog().textviewer("estoy dentro de getData///cookiejar y se va a enviar a Getsoup", str_soup)
        #str_soup = str(soup)
        #getSoup(str_soup)
        
        ####################
        #soup_str = str(soup)
        #xbmcgui.Dialog().textviewer("estoy dentro de getData///cookiejar", soup_str)
        #items_info = soup.find('items')
        #xml_data = ""
        #if items_info:
        #    item_tags = items_info.find_all('item')
        #    xml_data = '\n'.join(str(tag) for tag in item_tags)
        #    xml_data = re.sub(r"<link/>(.*)", r"<link>\g<1></link>", xml_data)
        #    xml_data = "<items_info>" '\n' + xml_data + '\n' "</items_info>"
       
        #xml_start = xml_data.find('<items_info>')
        #xml_end = xml_data.find('</items_info>', xml_start)
        #if xml_start != -1 and xml_end != -1:
        #    xml_data2 = xml_data[xml_start:xml_end + len('</items_info>')]
        #    xml_rectificada = '<items_info>'
        #    matches = re.findall(r'<item>(.*?)</item>', xml_data2, re.DOTALL)
        
        #    for match in matches:
        #        xml_rectificada = re.sub(r"\t", "", xml_rectificada)
        #        xml_rectificada += '<item>' + match + '</item>'

        #   xml_rectificada += '</items_info>'
        #    xbmcgui.Dialog().textviewer("Resultado cookiejar de element retificada y de aqui a getitems", xml_rectificada)
        #    try:
        #        xml = ElementTree.fromstring(xml_rectificada)
        #        #tree = ET.ElementTree(ET.fromstring(xml_rectificada))
        #        #root = tree.getroot()
        #        #elementos = list(root)
        #        xbmcgui.Dialog().textviewer("getitems va a recibir esto", str(xml))
        #        return xml
        #    except ElementTree.ParseError:
        #        pass
        #    except Exception as err:
        ...
        if k in regexs:
            ...
            if m['expres'] != '':
                if '$LiveStreamCaptcha' in m['expres']:
                    ...
                    val = ''
                    if link != '':
                        reg = re.compile(m['expres']).search(link)  # Utilizar el patrón como cadena de texto
                        if reg:
...
def addDir(name, url, mode, iconimage, fanart, description, genre, date, credits, showcontext=False, regexs=None, reg_url=None, allinfo={}):
    ...
    """
    #xbmcgui.Dialog().textviewer("estamos en addDir con el XML", str(name))
...
def addLink(url, name, iconimage, fanart, description, genre, date, showcontext, playlist, regexs, total, setCookie="", allinfo={}):
    #xbmcgui.Dialog().textviewer("estoy dentro de addLink", str(name))
